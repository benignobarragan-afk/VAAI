...
const fin_orde_comp = (async (req, res) => {
    ...
    SELECT p.user_id, p.id as id_cent, c.campus, c.descrip
	    FROM gen_dere_proy p INNER JOIN gen_centros c ON p.id = c.id_cent
    	WHERE p.user_id = ?
...
const fin_norde_comp = (async (req, res) => {
	...
	WHERE p.id_cent IN 
		(SELECT id 
			FROM gen_dere_proy
			WHERE user_id = ?) OR p.proyecto IN 
        (SELECT id 
            FROM gen_dere_proy 
            WHERE user_id = ?)
	ORDER BY c.campus, p.fondo, p.proyecto
    `
    const rows = await util.gene_cons(lcSQL, [req.userId, req.userId])
    ...
    return res.render("fin/fin_norde_comp", {rows, rows2, rows3, nomb_usua:req.nom_usu, skin:req.skin})
});
